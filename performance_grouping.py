...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan  3 22:08:19 2023
...
quiz_questions_merger["student_category"] = pd.cut(quiz_questions_merger["score"], bins=[-np.inf, 30.0, 70.0, 100], labels=["low", "mid", "high"])
quiz = quiz_questions_merger.groupby(['class_name','subject_name','student_category']).size().reset_index(name='counts')
...
subject_dropdown = dcc.Dropdown(options=quiz['subject_name'].unique(),
                            value= 0)
#subtopic_dropdown = dcc.Dropdown(options=quiz['subtopic_id'].unique(),
                           # value= 110)
#student_dropdown = dcc.Dropdown(options=quiz['student_id'].unique(),
                            #value= 110)

app.layout = html.Div(children=[
    html.H1(children='Quiz performance groupings'),
    html.P("Class:"),
    class_dropdown,
    html.P("Subject:"),
    subject_dropdown,
...
def generate_chart(selected_class, selected_subject):
   # filtered_quiz = quiz[quiz['marked'] == selected_mark]
    ...
    #fig = px.histogram(filtered_student, x="quiz_id", y="score_pct",
            # color='subtopic_name',histfunc='avg', barmode='group',
            #height=400)
    fig  = px.pie(filtered_student, values='counts', names='student_category')
